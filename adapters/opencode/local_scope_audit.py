"""OpenCode-facing LocalScope audit adapter."""
...
def local_scope_audit(
    *,
    ...
    """Run the high-level read-only LocalScope audit flow for OpenCode."""
...
def local_project_audit(**kwargs: Any) -> dict[str, Any]:
    """Compatibility alias for generic high-level audit naming."""
    return local_scope_audit(**kwargs)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run LocalScope as a read-only OpenCode tool.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Exception as error:  # noqa: BLE001 - OpenCode must always receive JSON.
